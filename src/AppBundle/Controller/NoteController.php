...
class NoteController extends FOSRestController
{
    ...
    public function getNotesAction(Request $request, ParamFetcherInterface $paramFetcher)
    {
        ...
        foreach($notes as $note) {
          if(is_numeric($note->getId())){
            $id = $note->getId();
            $params = array('id' => $id);
            $url = 'remove_notes';
            $deleteUrl = $this->generateUrl($url, $params);

            $note->addUrl('delete', $deleteUrl);
            $notesResult[] = $note;
          }
        }

        return new NoteCollection($notesResult, $offset, $limit);
    }
    ...
    {
        $result = $this->getNoteManager()->remove($id);

        $responseCode = $result ? Response::HTTP_NO_CONTENT : Response::HTTP_NOT_FOUND;
